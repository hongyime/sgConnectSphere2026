...
| **`/api/events` route collision, silently broke E01-S03 (SCRUM-18) GET listing** | Discovered post-Sprint 1 while scoping SCRUM-27 | PR #56 (SCRUM-26) shipped `api/events.ts` for POST alongside the pre-existing `api/events/index.ts` from PR #42 (SCRUM-18, E01-S03 hide internal planning info) for GET. Vercel silently deploys both and the parent file wins, so `GET /api/events` returned 405 in production for the whole of Sprint 1 and the organiser browse-events screen backed by ClientEvents.tsx was broken without anyone noticing. Fix landed in PR #78 (branch `fix/SCRUM-42-events-routing-collision`, name inherited from an early miscount of the affected Jira ticket &mdash; SCRUM-42 in Jira is E05-S03 venue calendar and is unrelated): consolidate into one method-dispatched handler, delete `events/index.ts`, add a pre-commit hook (`scripts/check_api_routes.py`) that fails on `foo.ts` + `foo/index.ts` collisions and holds the file count at 11 so a new feature never lands cap-blocked. Records ADR-014, BDR T-56. Postplan: <https://4ovp804r75sa.postplan.dev> (v1; ticket mis-attribution corrected in this row). |
...
| **Verify draft-privacy exclusion once E03 wires the Coordinator screen to real data** | Discovered while implementing SCRUM-27 | SCRUM-27 added `AND (e.status &lt;&gt; 'draft' OR e.organiser_id = $requester)` to `eventVisibility.listEvents`/`getEvent` so a colleague organiser (and, once it exists, a real Coordinator query) can't see another organiser's draft. Today this is unreachable from any real Coordinator screen — `Coordinator.tsx` (SCRUM-96, shipped Done as mock data) still reads `mocks.ts`, not the API. E03 (SCRUM-32 through SCRUM-37, all in Sprint 2, unassigned) is what will eventually wire a real Coordinator review flow to this same query path; whoever picks up SCRUM-34 (\"Decide on an event request\") should confirm the exclusion still holds once real data flows through. See BDR T-57 for the related SCRUM-27 draft-scope decision. |
