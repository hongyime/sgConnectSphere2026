...
## E02-S04 — Classify an event by category

- **Sprint**:
- **Points**: 1
- **BDR references**: C-32, O-13
- **Owner**:

### User story

As an Event Coordinator, I want to assign a category to an event from a predefined list so that events are classified to support filtering and reporting.

### Acceptance criteria

#### Scenario 1 — Category saved

Given I am creating or editing an event When I select a category from ConnectSphere's list Then it is saved against the event

#### Scenario 2 — New category available

Given ConnectSphere adds a new category When I next create an event Then the new category is available for selection

### Checklist

_No checklist recorded._

## E02-S05 — Reuse a previous event's details
...
As an Event Organiser, I want to create a new request by copying a previous event so that I do not re-enter the same information for a recurring event.

### Acceptance criteria

#### Scenario 1 — Previous event copied

Given I have a previous event When I choose to copy it Then a new draft is created with the requirements copied and the dates left blank

#### Scenario 2 — Missing reference flagged

Given the copied event referenced a venue or equipment item that no longer exists When the draft is created Then that requirement is flagged for review rather than copied silently

### Checklist

_No checklist recorded._

## E02-S06 — Attach supporting documents

- **Sprint**:
- **Points**: 3
- **BDR references**: C-31, T-40, O-08
- **Owner**:

### User story

As an Event Organiser, I want to attach documents such as floor plans and programmes to my event so that ConnectSphere has the materials it needs.

### Acceptance criteria

#### Scenario 1 — Permitted file attached

Given I am viewing an event I own When I upload a PDF, PNG or JPEG of 300MB or less Then it is attached to the event and visible to the assigned internal users

#### Scenario 2 — Oversized or wrong type rejected

Given I upload a file larger than 300MB or of a type other than PDF, PNG or JPEG When I confirm the upload Then it is rejected and the permitted types and size limit are stated

#### Scenario 3 — Attachment deleted

Given a document is attached to an event When I delete it Then it is removed and the deletion is recorded in the activity log

### Checklist

- Attach a PDF, PNG or JPEG of 300MB or less to an event I own
- Be rejected with the permitted types and limit stated when the file is too large or the wrong type
- See attached documents as an assigned internal user
- Delete an attachment, with the deletion recorded in the activity log
