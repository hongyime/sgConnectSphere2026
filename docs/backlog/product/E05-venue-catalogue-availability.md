...
- **Sprint**:
- **Points**: 5
- **BDR references**: C-18, C-38
- **Owner**:

### User story

As a Venue Staff member, I want to record the setup and turnaround time each venue requires so that consecutive bookings leave enough time for preparation and reset.

### Acceptance criteria

#### Scenario 1 — Availability extended by turnaround

Given a venue has a defined setup and turnaround time When its availability is calculated Then the unavailable period extends before and after the published event times

#### Scenario 2 — Insufficient turnaround flagged

Given a booking request would leave less than the required turnaround after an existing booking When the request is submitted Then it is flagged as conflicting
